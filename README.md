...
アーカイブ内および D&D 一時ビューアの画像形式: PNG, JPG, GIF, WebP, BMP, AVIF, JFIF, HEIC, HEIF, TIFF

### 閲覧モード

- **ページめくり / シームレススクロール** — EPUB の表示方式を切り替え可能
- **目次連動のスクロール分割** — EPUB の目次リンクを章境界として扱い、先頭の表紙/目次ページが目次に無い場合は Spine/Nav 構造から仮想「表紙」「目次」を補完して重複表示を防ぐ
...
- 📂 **ドラッグ＆ドロップ** でファイルを開く（画像ファイル/画像フォルダは一時ビューアとして表示し、履歴・ライブラリには保存しない）
- 🖥️ **フルスクリーン** 対応
