...
- [ ] 変更した箇所の参照元をすべて確認したか
- [ ] バージョン更新が必要かどうか確認したか（バージョンナンバリング規則参照）
...
アプリケーションのバージョンは `assets/constants/app-info.js` の `VERSION` で一元管理する（SSOT）。

### セマンティクス
`Ver{MAJOR}.{MIDDLE}.{MINOR}` （例: Ver1.0.0）
- **MAJOR（メジャー）**: 後方互換性のない大規模変更。ユーザーが明示的に更新を指示した場合のみ進める。
- **MIDDLE（ミドル）**: 一定の機能開発が完了し、ユーザーが「開発完了」を宣言したら進める。16進数表記（0-9, A-F）。
- **MINOR（マイナー）**: ユーザーが改修を依頼し、プッシュまで進めた場合に自動で進める。16進数表記（0-9, A-F）。

### 桁あふれ対策
MINOR または MIDDLE が F(15) を超える場合、次の桁に繰り上げるのではなく桁を増やす。
例: `1.0.F` → `1.0.10` → `1.0.11` → ... → `1.0.1F` → `1.0.20`

### 更新タイミング
| トリガー | 更新対象 |
|----------|----------|
| 改修依頼を受けてコード変更 → プッシュ | MINOR +1 |
| ユーザーが「開発完了」と発言 | MIDDLE +1, MINOR リセット |
| ユーザーがメジャー更新を明示 | MAJOR +1, MIDDLE/MINOR リセット |
